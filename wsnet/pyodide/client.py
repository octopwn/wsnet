...
class WSNetworkUDP:
		...
		try:
			while not self.disconnected_evt.is_set():
				try:
					...
					await self.in_q.put((cmd.data, (cmd.clientip, cmd.clientport), None))
				except asyncio.CancelledError:
					return
				except Exception as e:
					await self.in_q.put((None, None, e))
					return
		finally:
			await self.terminate()
	...
	async def __handle_out(self):
		try:
			connectiontoken = b'\x00' * 16
			while not self.disconnected_evt.is_set():
				outgoing_struct = await self.out_q.get()
				#print('OUT %s' % data)
				if outgoing_struct is None:
					return
				
				data, addr = outgoing_struct
				if data is None or data == b'':
					return

				if len(data) < 286295:
					cmd = WSNServerSocketData(self.token, connectiontoken, data, clientip = str(addr[0]), clientport = addr[1])
					js.sendWebSocketData(self.ws, cmd.to_bytes())
				else:
					# This is UDP this will explode... TODO: fix this
					# need to chunk the data because WS only accepts max 286295 bytes in one message
					data = io.BytesIO(data)
					while True:
						chunk = data.read(286200)
						if chunk == b'':
							break
						cmd = WSNServerSocketData(self.token, connectiontoken, chunk, clientip = str(addr[0]), clientport = addr[1])
						js.sendWebSocketData(self.ws, cmd.to_bytes())
	...
	async def connect(self):
		try:
			await asyncio.wait_for(self.connected_evt.wait(), 10)
			cmd = WSNConnect(self.token, 'UDP', self.ip, self.port)
			js.sendWebSocketData(self.ws, cmd.to_bytes())
